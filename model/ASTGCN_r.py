...
#         print("x.permute(0, 3, 2, 1).shape: ", x.permute(0, 3, 2, 1).shape)
#         print("torch.matmul(x.permute(0, 3, 2, 1), self.U1).shape: ", torch.matmul(x.permute(0, 3, 2, 1), self.U1).shape)
...
class ASTGCN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_for_predict, len_input, num_of_vertices):
        ...
        # self.final_conv = nn.Conv2d(int(len_input/time_strides), num_for_predict, kernel_size=(2, nb_time_filter))
...
#         output = self.final_conv(x.permute(0, 3, 1, 2)).permute(0, 2, 1, 3)
#         # (b,N,F,T)->(b,T,N,F)-conv<2,F>->(b,c_out*T,N,2)->(b,N,T,2)
