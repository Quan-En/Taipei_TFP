...
def load_graphdata_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True, with_covid=False):
    '''
    这个是为PEMS的数据准备的函数
    ...
    if with_covid:
        covid_train_x = train_x[:,0,2:,-1].reshape(-1, 7, 4)
        train_x = train_x[:,:,:2,:]
        
        covid_val_x = val_x[:,0,2:,-1].reshape(-1, 7, 4)
        val_x = val_x[:,:,:2,:]
        
        covid_test_x = test_x[:,0,2:,-1].reshape(-1, 7, 4)
        test_x = test_x[:,:,:2,:]
    ...
    if with_covid:
        train_covid_data_tensor = torch.from_numpy(covid_train_x).type(torch.FloatTensor).to(DEVICE) # (B, 7(week), 4(features))
        train_dataset = torch.utils.data.TensorDataset(train_x_tensor, train_target_tensor, train_covid_data_tensor)
    else:
    ...
    if with_covid:
        val_covid_data_tensor = torch.from_numpy(covid_val_x).type(torch.FloatTensor).to(DEVICE) # (B, 7(week), 4(features))
        val_dataset = torch.utils.data.TensorDataset(val_x_tensor, val_target_tensor, val_covid_data_tensor)
    else:
    ...
    if with_covid:
        test_covid_data_tensor = torch.from_numpy(covid_test_x).type(torch.FloatTensor).to(DEVICE) # (B, 7(week), 4(features))
        test_dataset = torch.utils.data.TensorDataset(test_x_tensor, test_target_tensor, test_covid_data_tensor)
    else:
...
def compute_val_loss_mstgcn_lstm(net, lstm_net, val_loader, criterion, sw, epoch, limit=None):
    '''
    for rnn, compute mean loss on validation set
    :param net: model
    :param val_loader: torch.utils.data.utils.DataLoader
...
def evaluate_on_test_mstgcn_lstm(net, lstm_net, est_loader, test_target_tensor, sw, epoch, _mean, _std):
    '''
    for rnn, compute MAE, RMSE, MAPE scores of the prediction for every time step on testing set.
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(test_loader):
            ...
            prediction.append(final_outputs.detach().cpu().numpy())
...
def predict_and_save_results_mstgcn_lstm(net, lstm_net, data_loader, data_target_tensor, global_step, _mean, _std, params_path, type):
    '''
